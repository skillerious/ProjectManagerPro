(() => {
    ...
    function createLogViewerController(options = {}) {
        ...
        function updateSourceFilterOptions() {
            ...
            const sources = Array.from(new Set(state.entries.map((entry) => entry.source))).sort((left, right) => left.localeCompare(right));
        ...
        function renderEntries() {
            ...
            const rendered = state.filteredEntries.slice(0, MAX_RENDERED_ROWS).map((entry) => {
                ...
                return `
                    <button type="button" class="log-viewer-row${activeClass}${faultClass}" data-entry-id="${entry.id}">
        ...
        function bindEvents() {
            state.elements['log-viewer-search-input'].addEventListener('input', () => applyFilters());
            state.elements['log-viewer-level-filter'].addEventListener('change', () => applyFilters());
            state.elements['log-viewer-source-filter'].addEventListener('change', () => applyFilters());
            state.elements['log-viewer-fault-toggle'].addEventListener('change', () => applyFilters());
            ...
            document.addEventListener('keydown', (event) => {
                ...
                if (toSafeString(event.key, '').toLowerCase() === 'escape') {
                    event.preventDefault();
        ...
        async function open() {
            ...
            await navigateToView('diagnostics');
