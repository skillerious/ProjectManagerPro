...
const fs = require('fs');
const MAX_COMMAND_INPUT_LENGTH = 2048;
const MAX_VALIDATION_INPUT_LENGTH = 4096;
const MAX_GIT_REF_LENGTH = 255;
const MAX_GIT_REMOTE_URL_LENGTH = 2048;
const MAX_GIT_FILE_PATH_LENGTH = 1024;
...
function parseAllowedRunCommand(command) {
  ...
  if (!normalizedCommand || normalizedCommand.length > MAX_COMMAND_INPUT_LENGTH) {
    return null;
  }
...
function validateGitPath(projectPath) {
  ...
  if (!trimmedPath || trimmedPath.length > MAX_VALIDATION_INPUT_LENGTH || /[\0\r\n]/.test(trimmedPath)) {
    return { valid: false, error: 'Invalid project path' };
  }
...
function validateGitRefName(refName, label = 'Git reference') {
  ...
  if (trimmedRef.length > MAX_GIT_REF_LENGTH || /[\0\r\n]/.test(trimmedRef)) {
...
function validateGitHash(hash) {
  ...
  if (trimmedHash.length > MAX_GIT_REF_LENGTH || /[\0\r\n]/.test(trimmedHash)) {
...
function validateGitRemoteUrl(url) {
  ...
  if (!trimmedUrl || trimmedUrl.length > MAX_GIT_REMOTE_URL_LENGTH) {
...
function validateGitFilePathInput(filePath) {
  ...
  if (
    !trimmedPath
    || trimmedPath.length > MAX_GIT_FILE_PATH_LENGTH
    || /["'`$&|;()<>\r\n\0]/.test(trimmedPath)
  ) {
    return { valid: false, error: 'Invalid file path' };
  }
